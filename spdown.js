...
const client_id = '7e9b882432e644ee9cbe638b83ac05e2';
const client_secret = '03b18c48971e421a9f0b59dcdc516b13';
...
const getSpotifyAccessToken = async () => {
    try {
        ...
        throw new Error('Spotify token alınırken hata oluştu. // Error fetching Spotify access token');
    }
};
...
const getSpotifyTrackDetails = async (url) => {
    try {
        ...
        throw new Error('Spotify parça bilgileri alınırken hata oluştu. // Error fetching Spotify track details');
    }
};
...
const getDownloadLink = async (gid, track_id) => {
    try {
        ...
        throw new Error('İndirme bağlantısı alınırken hata oluştu. // Error fetching download link');
    }
};
...
const getTrackMetadata = async (trackId, accessToken) => {
    try {
        ...
        throw new Error('Parça meta verileri alınırken hata oluştu. // Error fetching track metadata');
    }
};
...
app.get('/spotify/down', async (req, res) => {
    ...
    if (!spotifyUrl) {
        return res.status(400).json({ status: false, message: 'Spotify URL Bulunamadı // Spotify URL is required' });
    }
    ...
    try {
        ...
        if (!downloadTask.download_url) {
            return res.status(500).json({ status: false, message: 'İndirme bağlantısı alınamadı // Failed to retrieve download link'});
        }
        ...
        res.status(500).json({ status: false, message: error.message || 'Bir hata oluştu // An error occurred' });
    }
});

app.listen(PORT, () => {
    console.log(`Sunucu şu portta çalışıyor:${PORT} // Server is running on port ${PORT}`);
});
