...
const handleTrackDownload = async (trackUrl) => {
  const downloadUrl = `https://spotify-kringof.vercel.app/spotify/down?url=${trackUrl}`;
    ...
    if (spotifyUrl) {
      ...
      if (isPlaylist) {
        ...
        return res.status(200).json({
          ...
          developerCredit: 'https://t.me/Kringof_Api'
        });
      } else if (isAlbum) {
        ...
        return res.status(400).json({ error: 'Geçersiz Spotify URL si // Invalid Spotify URL' });
      }
    } else if (query) {
      const accessToken = await getAccessToken();
      if (limit > 50) {
        ...
        const trackDetailsList = combinedTracks.map((track, index) => {
          const previewUrl = track.preview_url || 'Önizleme mevcut değil // No preview available';
          const image = track.album.images.length > 0 ? track.album.images[0].url : 'Resim Bulunamadı // No image available';
        ...
        if (tracks.length > 0) {
          const trackDetailsList = tracks.map((track, index) => {
            const previewUrl = track.preview_url || 'Önizleme Bulunamadı // No preview available';
            const image = track.album.images.length > 0 ? track.album.images[0].url : 'Resim Bulunamadı // No image available';
            const artists = track.artists.map(artist => ({
              name: artist.name,
              spotifyUrl: artist.external_urls.spotify,
          ...
          return res.status(404).json({ error: 'Parça Bulunamadı // No tracks found' });
        }
      }
    } else {
      return res.status(400).json({ error: ' Eksik sorgu parametresi (q) veya Spotify URL’si (url) // Missing query parameter (q) or Spotify URL (url)' });
    }
  } catch (error) {
    console.error('Error:', error);
    return res.status(500).json({ error: 'Erişim Sorunu // Internal server error' });
  }
});
